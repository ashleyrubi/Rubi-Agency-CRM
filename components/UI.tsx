...
import { LucideIcon, Search, X, AlertTriangle, Lock, ChevronDown } from 'lucide-react';
...
const baseInputStyle = "w-full px-4 py-3 bg-gray-50 dark:bg-[#0f0f11] border border-gray-100 dark:border-dark-border rounded-xl focus:ring-2 focus:ring-brand-pink focus:outline-none font-bold text-sm text-gray-900 dark:text-white transition-all appearance-none placeholder:text-gray-400 dark:placeholder:text-gray-600 selection:bg-brand-pink/30";

export const Input: React.FC<React.InputHTMLAttributes<HTMLInputElement>> = (props) => (
  <input 
    className={`${baseInputStyle} ${props.type === 'date' ? '[color-scheme:light] dark:[color-scheme:dark]' : ''} ${props.className || ''}`} 
    {...props} 
  />
);
...
export const Select: React.FC<React.SelectHTMLAttributes<HTMLSelectElement>> = (props) => (
  <div className="relative w-full">
    <select 
      className={`${baseInputStyle} pr-10 cursor-pointer ${props.className || ''}`} 
      {...props} 
    />
...
export const Textarea: React.FC<React.TextareaHTMLAttributes<HTMLTextAreaElement>> = (props) => (
  <textarea className={`${baseInputStyle} min-h-[100px] font-medium ${props.className || ''}`} {...props} />
);
