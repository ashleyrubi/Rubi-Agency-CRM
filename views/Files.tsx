  ...
  return (
    <div className="p-10">
      <PageHeader 
        ...
        actions={
          <div className="flex flex-col sm:flex-row gap-3 w-full sm:w-auto">
            <Select value={selectedClientId} onChange={e => setSelectedClientId(e.target.value)} className="w-full sm:w-48">
              <option value="">Choose Client</option>
              {clients.map(c => <option key={c.id} value={c.id}>{c.company}</option>)}
            </Select>
            <Button variant="primary" icon={Upload} disabled={!selectedClientId || isUploading} className="relative w-full sm:w-auto">
      ...
      <Card className="overflow-hidden">
        <div className="p-6 border-b border-gray-100 dark:border-dark-border bg-gray-50/50 dark:bg-dark-bg/50 flex flex-col sm:flex-row justify-between items-center gap-4">
          ...
          <div className="text-[10px] font-black text-gray-400 uppercase tracking-widest flex items-center shrink-0">
            ...
            return (
              <tr key={file.id} className="hover:bg-gray-50/50 dark:hover:bg-dark-bg/50 transition-colors group">
                <td className="px-6 py-4 flex items-center">
                  <div className="w-10 h-10 bg-gray-100 dark:bg-dark-bg rounded-xl flex items-center justify-center text-gray-400 group-hover:text-brand-pink transition-colors mr-4 shadow-sm border border-transparent dark:border-dark-border">
        ...
        {filteredFiles.length === 0 && <EmptyState icon={File} message="Empty Library" submessage="Upload assets to share them across the agency." />}
      </Card>
    </div>
  );
};
