return (
          ...
          <>
            ...
            <ToolbarItem label="Status filter">
              <div className="flex flex-wrap gap-3">
                {(['Not Started', 'In Progress', 'Complete'] as StatusValue[]).map(
                  (status) => (
  ...
  >
    ...
    {/* TABLE WRAPPER – THIS IS WHAT FIXES THE SCROLL */}
    <div className="flex-1 min-h-0 overflow-x-auto overflow-y-auto">
      <Card className="border-none shadow-2xl bg-white dark:bg-dark-card h-full">
        <div className="h-full overflow-auto">
          <Table
            className="min-w-[2600px] sheets-style-table"
            headers={[
              'Project',
              'Brief Status',
              'Content Status',
          ...
          >
            {sortedTasks.map((task) => (
              <tr key={task.id}>
                {/* your existing row cells stay exactly as they are */}
              </tr>
            ))}
          </Table>
        </div>
