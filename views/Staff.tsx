...
const StaffView: React.FC = () => {
  ...
  return (
    <div className="p-6 md:p-10">
            ...
            <tr key={member.id} className="hover:bg-gray-50/50 dark:hover:bg-dark-bg/50 transition-colors group">
              ...
              <td className="px-6 py-4 text-right">
                <div className="flex justify-end gap-2">
                  <button onClick={() => handleOpenEdit(member)} className="p-2 bg-gray-50 dark:bg-dark-bg text-gray-400 hover:text-brand-pink rounded-lg transition-colors">
                    <Edit3 className="w-4 h-4" />
                  </button>
                  <button onClick={() => handleDelete(member.id!)} className="p-2 bg-gray-50 dark:bg-dark-bg text-gray-400 hover:text-red-500 rounded-lg transition-colors">
      ...
      {/* Mobile/Tablet View normalized to p-5 and space-y-4 */}
      <div className="lg:hidden space-y-4">
        {sortedStaff.map(member => (
          <Card key={member.id} className="p-5">
            <div className="flex justify-between items-start mb-4">
              <div className="flex items-center gap-3">
                <div className="w-10 h-10 rounded-xl bg-brand-pink text-white font-black flex items-center justify-center shadow-sm">
                  {member.name.charAt(0)}
                </div>
                <div>
                  <h3 className="text-base font-black text-gray-900 dark:text-white">{member.name}</h3>
                  <div className="flex items-center gap-2 mt-0.5">
                    <span className="text-[9px] font-black text-brand-pink bg-brand-pink/5 px-1.5 py-0.5 rounded border border-brand-pink/10">
                      #{member.employeeNumber || '--'}
                    </span>
                    <span className="text-[10px] text-gray-400 font-bold uppercase tracking-tight">{member.department}</span>
                  </div>
                </div>
              </div>
              <div className="flex gap-1">
                <button onClick={() => handleOpenEdit(member)} className="p-2.5 text-gray-400 hover:text-brand-pink transition-colors">
                  <Edit3 className="w-4 h-4" />
                </button>
                <button onClick={() => handleDelete(member.id!)} className="p-2.5 text-gray-400 hover:text-red-500 transition-colors">
                  <Trash2 className="w-4 h-4" />
                </button>
              </div>
            </div>
            ...
            <div className="space-y-2 mb-4">
              <div className="text-[10px] text-gray-400 font-black uppercase tracking-widest">{member.jobRole || member.role}</div>
              <div className="flex items-center text-xs font-bold text-gray-600 dark:text-gray-400">
                <Mail className="w-3.5 h-3.5 mr-2 opacity-40" /> {member.email}
              </div>
              {member.phone && (
                <div className="flex items-center text-xs font-bold text-gray-600 dark:text-gray-400">
                  <Phone className="w-3.5 h-3.5 mr-2 opacity-40" /> {member.phone}
                </div>
              )}
            </div>

            <div className="pt-4 border-t border-gray-50 dark:border-dark-border flex items-center justify-between">
              <StatusBadge status={member.status} />
              <div className="flex items-center gap-1.5">
                ...
                <span className={`text-[9px] font-black uppercase tracking-widest ${member.linked ? 'text-green-600' : 'text-gray-400'}`}>
